

<template>
    <!--bara mall för hur man kan göra-->

<div>
    <div>
                  <h4>Navigera runt skärmen</h4>
              <b-card-group deck>
                <!--<b-card img-src="https://placekitten.com/1000/300" img-alt="Card image" img-top>
          <b-card-text>
            Some quick example text to build on the card and make up the bulk of the card's content.
          </b-card-text>
        </b-card>-->

                <b-card img-src="https://placekitten.com/1000/300" img-alt="Card image" img-bottom>
              <b-card-text>
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Nostrum sit sint minus at ad sapiente cupiditate incidunt soluta, consequatur, quidem quia aperiam nulla, vitae sunt asperiores tenetur? Porro, illum eveniet!
            </b-card-text>
            </b-card>
          </b-card-group>
    </div>
    <div class="mt-4">
      <h4>Left and Right (or Start and End)</h4>
      <b-card img-src="https://placekitten.com/300/300" img-alt="Card image" img-left class="mb-3">
        <b-card-text>
          Some quick example text to build on the card and make up the bulk of the card's content.
        </b-card-text>
      </b-card>

      <b-card img-src="https://placekitten.com/300/300" img-alt="Card image" img-right>
        <b-card-text>
          Some quick example text to build on the card and make up the bulk of the card's content.
        </b-card-text>
      </b-card>
    </div>
  </div>
  <router-link to="/android" @click="scrollToTop">Back to Android Guides</router-link>
</template>

<script>
export default { // Detta är för komma längst upp på sidan man router-linka till.
methods: {
    scrollToTop() {
        window.scrollTo(0, 0); // Scrolla till (x, y) = (0, 0)
    }
}
}
</script>
